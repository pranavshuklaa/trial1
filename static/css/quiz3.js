...
submit.addEventListener('click',()=>{
    ...
    if(checkedAnswer==1){
        ...
        <h3>End of Test</h3>
        `
        console.log("You scored ${score}/25")
        showScore.innerHTML=`
         Description
        `;
        ...
        console.log(val)
        if(val>=0 && val<13){
            submit.innerHTML=`<h4>None to slight</h4>`
        }
        else if(val>=13 && val<=16){
            submit.innerHTML=`<h4>Mild </h4>`
        }
        else if(val>17 && val<=20){
            submit.innerHTML=`<h4>Moderate </h4>`
        }
        else{
            submit.innerHTML=`<h4>Severe</h4>`;
        }
